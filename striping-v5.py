    ...
    for x in range(BG_CARDINALITY):
        ...
        if counter % 2 == 0:
            ...
            res.append(pool.apply_async(aux_gen_distortions, args=(
                ...
                rand_int,
                TGT_SIZE[0],
                non_overlap_rand_bg(test_dir),"gaussian",)))
            res.append(pool.apply_async(aux_gen_distortions, args=(
                test_dir,
                src,
                timestamp,
                rand_int,
                TGT_SIZE[0],
                non_overlap_rand_bg(test_dir),"brightness",)))
            res.append(pool.apply_async(aux_gen_distortions, args=(
                test_dir,
                src,
                timestamp,
                rand_int,
                TGT_SIZE[0],
                non_overlap_rand_bg(test_dir),"contrast",)))
            res.append(pool.apply_async(aux_gen_distortions, args=(
                test_dir,
                src,
                timestamp,
                rand_int,
                TGT_SIZE[0],
                non_overlap_rand_bg(test_dir),"color",)))
        ...
        for sz in [180, 190, 200]:
            if x % 2 == 0:
                ...
                res.append(pool.apply_async(aux_gen_distortions, args=(
                    ...
                    rand_int,
                    sz,
                    non_overlap_rand_bg(train_dir),"gaussian")))
                res.append(pool.apply_async(aux_gen_distortions, args=(
                    train_dir,
                    src,
                    timestamp,
                    rand_int,
                    sz,
                    non_overlap_rand_bg(train_dir),"brightness")))
            else:
                res.append(pool.apply_async(aux_gen_distortions, args=(
                    test_dir,
                    src,
